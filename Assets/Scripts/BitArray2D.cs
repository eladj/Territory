...
public class BitArray2D
{
    ...
    public void Or(BitArray2D other)
    {
        this._bitarray.Or(other.BitArray);
    }
    ...
    /// <summary>
    /// Returns the number of true bits in the BitArray
    /// </summary>
    /// <returns>Number of true bits</returns>
    public int Sum(){
        int sum = 0;
        for (int i=0; i < this._bitarray.Count; i++){
            sum += this._bitarray.Get(i) ? 1 : 0;
        }
        return sum;
    }
}
